...
var server = http.createServer(function(req, res) {
    ...
    if (qdata.appid === 'app-id' && qdata.action === 'update') {
        ...
        if (total < 40) {
            var html = buildHtml(Number(qdata.total), qdata.time, Number(qdata.usedtoday), qdata.hotwater);
...
function buildHtml(total, time, usedtoday, hotwater) {
...
//    var body = '<h1>Solar Today: ' + total + ' kWh</h1> <h2>@' + time + '</h2>' + '<h2>Hot Water Status</h2> 
    var body = '<h1>@ ' + time + '</h1> <h2>Solar Generated Today: ' + total + ' kWh </h2>' + '<h1>Hot Water Status</h1> <h2>Solar Used Today: ' + usedtoday.toLocaleString() + ' Wh</h2> <h2>Shower Status: ' + hotwater + '</h2>' 
